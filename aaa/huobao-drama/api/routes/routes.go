...
func SetupRouter(cfg *config.Config, db *gorm.DB, log *logger.Logger, localStorage interface{}) *gin.Engine {
	...
	api := r.Group("/api/v1")
	{
		...
		dramas := api.Group("/dramas") // 剧本
		{
			...
			dramas.GET("/:id/characters", dramaHandler.GetCharacters) //章节
			dramas.PUT("/:id/characters", dramaHandler.SaveCharacters)
			dramas.PUT("/:id/episodes", dramaHandler.SaveEpisodes) //剧集
			dramas.PUT("/:id/progress", dramaHandler.SaveProgress)
			dramas.GET("/:id/props", propHandler.ListProps) // Added prop list route  //道具
		}
